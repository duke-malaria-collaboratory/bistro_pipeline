...
humans <- read_csv(snakemake@input[[2]], col_types = 'c') %>% suppressMessages()
mozzies <- read_csv(snakemake@input[[3]], col_types = 'c') %>% suppressMessages()
mozzie_ids_all <- read_csv(snakemake@input[[4]], col_types = 'c') %>% pull(SampleName) %>% suppressMessages()
